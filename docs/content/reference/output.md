...
describes what users get. Pick a format with `-o`, or let be choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
be <command> -o table   # aligned columns for reading
be <command> -o jsonl   # one JSON object per line, for piping
be <command> -o json    # a single JSON array
be <command> -o csv     # spreadsheet friendly
be <command> -o tsv     # tab-separated
be <command> -o url     # just the URL column
be <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
be <command> --fields id,title,url
```
...
```bash
be <command>            # a table, because this is a terminal
be <command> | wc -l    # JSONL, because this is a pipe
```
