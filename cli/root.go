// Package cli builds the be command tree on top of the betterexplained library.
package cli
...
type App struct {
	client *betterexplained.Client
	cfg    betterexplained.Config
...
func Root() *cobra.Command {
	app := &App{cfg: betterexplained.DefaultConfig()}

	root := &cobra.Command{
		Use:   "be",
		Short: "Read BetterExplained math and programming articles",
		Long: `be reads BetterExplained via the WordPress REST API.
No API key is required. Records are returned as table, JSON, JSONL,
CSV, TSV, or URLs.

be is an independent tool and is not affiliated with BetterExplained or Kalid Azad.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = betterexplained.NewClient(a.cfg)
	return nil
}
